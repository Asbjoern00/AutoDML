...
def run_experiment(data):
    model_wrapper = ModelWrapper(in_=25, hidden_size=100, n_shared=3, n_not_shared=2, type_="separate_nets")
    model_wrapper.train_riesz_head(data, lr=1e-3, train_shared_layers=True)
    model_wrapper.train_outcome_head(data, lr=1e-3, wd=1e-3, train_shared_layers=True)
...
estimates.to_csv("doper_neural_nets/results/separate_nets.csv", index=False)
