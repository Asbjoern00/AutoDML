...
for i in range(K):
    ...
    dragon_net = DragonNetWrapper.create_dragon_net(weight_decay=1e-3)
    base_dragon_net = DragonNetWrapper.create_base_dragon_net(weight_decay=1e-3)
    ...
    dragon_net_error = sum(estimate - truth for estimate, truth in zip(estimates, truths)) / len(estimates)
    base_dragon_net = sum(estimate - truth for estimate, truth in zip(base_estimates, truths)) / len(base_estimates)

    print(f'BaseDragonNet Mean Abs Error: {base_dragon_net}')
