...
def run():
    ...
    if os.path.isfile("ase_experiment/lasso_experiment/Results/cross_fit_results.csv"):
        ...
        n_already_run = np.argmax(already_run[:,7] == 0)


        est_plugin[:n_already_run] = already_run[:n_already_run, 1]

        est_indirect[:n_already_run] = already_run[:n_already_run, 2]
        var_indirect[:n_already_run] = already_run[:n_already_run, 3]
        lower_ci_indirect[:n_already_run] = already_run[:n_already_run, 4]
        upper_ci_indirect[:n_already_run] = already_run[:n_already_run, 5]

        est_riesz[:n_already_run] = already_run[:n_already_run, 6]
        var_riesz[:n_already_run] = already_run[:n_already_run, 7]
        lower_ci_riesz[:n_already_run] = already_run[:n_already_run, 8]
        upper_ci_riesz[:n_already_run] = already_run[:n_already_run, 9]
