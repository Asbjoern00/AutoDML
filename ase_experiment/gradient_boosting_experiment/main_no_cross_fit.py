...
from ase_experiment.gradient_boosting_experiment.cross_validate import cross_validate

for number_of_samples in [125, 500, 2000]:
    np.random.seed(2131)
    ...
    for i in range(1000):
        ...
        print((plugin_mse*number_of_samples)**0.5, (propensity_mse*number_of_samples)**0.5, (riesz_mse*number_of_samples)**0.5)
    ...
    np.savetxt(
        f"ase_experiment/gradient_boosting_experiment/results/no_cross_fit_results_{number_of_samples}.csv",
        results,
        delimiter=",",
        header=",".join(headers),
        comments="",
