...
xg_2000_ate_cross <- read_csv("ate_experiment/gradient_boosting_experiment/results/cross_fit_results_500.csv")%>%
  mutate(riesz_residual = (riesz_estimate-truth)^2, propensity_residual = (propensity_estimate-truth)^2) %>%
  summarise(riesz_mse = mean(riesz_residual), riesz_sd = sd(riesz_residual) / sqrt(1000), indirect_mse = mean(propensity_residual), indirect_sd = sd(propensity_residual) / sqrt(1000)) %>%
...
xg_500_ase_cross <- read_csv("ase_experiment/gradient_boosting_experiment/results/cross_fit_results_125.csv")%>%
  mutate(riesz_residual = (riesz_estimate-truth)^2, propensity_residual = (propensity_estimate-truth)^2) %>%
  summarise(riesz_mse = mean(riesz_residual), riesz_sd = sd(riesz_residual) / sqrt(1000), indirect_mse = mean(propensity_residual), indirect_sd = sd(propensity_residual) / sqrt(1000)) %>%
...
xg_2000_ase_cross <- read_csv("ase_experiment/gradient_boosting_experiment/results/cross_fit_results_125.csv")%>%
  mutate(riesz_residual = (riesz_estimate-truth)^2, propensity_residual = (propensity_estimate-truth)^2) %>%
  summarise(riesz_mse = mean(riesz_residual), riesz_sd = sd(riesz_residual) / sqrt(1000), indirect_mse = mean(propensity_residual), indirect_sd = sd(propensity_residual) / sqrt(1000)) %>%
...
data = rbind(xg_125_ate_cross, xg_500_ate_cross, xg_2000_ate_cross, xg_125_ase_cross, xg_500_ase_cross, xg_2000_ase_cross,
...
ggplot(data) +
  ...
  ggtitle('RMSE without cross-fitting')
