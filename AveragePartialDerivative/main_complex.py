...
def run(n_shared):
    ...
    for i in range(m):
        ...
        data = pd.read_csv(f"AveragePartialDerivative/BHP_data/redrawn_datasets/complex_f_with_linear_and_non_linear_confounders/data_{i}.csv")
        ...
        np.savetxt(
            f"AveragePartialDerivative/results/avg_derivative_dope_{n_shared}_complex.csv",
            results,
            delimiter=",",
            header=",".join(headers),
            comments="",
