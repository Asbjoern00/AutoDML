...
def run(n_riesz):
    ...
    for rr_weight in rr_weights[::-1]:
        ...
        for i in range(n_run, m):
            ...
            data = Dataset(
                np.concatenate([data.outcomes, data.treatments, data.covariates], axis=1),
                outcome_column=0,
                treatment_column=1,
            )
            ...
            print(f"RR weight {rr_weight} without TMLE, n_shared = {n}")
            print(
                f"RMSE : {np.sqrt(np.mean((est_riesz[:i+1]-truths[:i+1])**2))}, coverage = {np.mean(covered_riesz[:i+1])}, bias : {(np.mean((est_riesz[:i+1]-truths[:i+1])))}, MAE = {(np.mean(np.abs((est_riesz[:i+1]-truths[:i+1]))))}"
            )
            print(i)
