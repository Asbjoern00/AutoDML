import torch
import numpy as np
from dope_neural_nets.representation_size.model_no_shared import ModelWrapper
...
def run_experiment(data):
    estimate_components = []
    model_wrapper = ModelWrapper(in_=25, hidden_size=100, n_layers=5)
    model_wrapper.train_outcome_head(data, lr=1e-3)
    model_wrapper.train_riesz_head(data, lr=1e-3)
...
estimates.to_csv("dope_neural_nets/representation_size/0_shared.csv", index=False)
